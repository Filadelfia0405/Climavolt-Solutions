...
export function Estimates() {
  ...
  const [items, setItems] = useState<InvoiceItem[]>([
    { id: '1', description: '', quantity: 1, unitPrice: 0, discountPercentage: 0 }
  ])
  ...
  const addItem = () => {
    setItems(prev => [...prev, { id: Date.now().toString(), description: '', quantity: 1, unitPrice: 0, discountPercentage: 0 }])
  }
    ...
    <div className="flex h-full flex-col p-4 bg-[#0B1120] overflow-y-auto pb-24">
      ...
      <header className="mb-6 flex items-center justify-between pt-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Generar Presupuesto</h1>
          </div>
        </div>
      ...
      <div id="invoice-content" className="max-w-4xl mx-auto w-full bg-slate-900 rounded-xl p-4 md:p-6 space-y-6">
        ...
        <div className="overflow-hidden rounded-lg border border-blue-900/50">
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-blue-900 text-xs text-white">
              <tr>
                <th className="px-4 py-2 font-bold w-1/2 md:w-2/5">DESCRIPCIÓN DEL SERVICIO / TRABAJOS REALIZADOS</th>
                ...
                <th className="px-2 py-2 font-bold text-right w-24">DESC. (%)</th>
                ...
                <tr key={item.id} className="border-b border-slate-800 last:border-0 bg-slate-800/50">
                  ...
                  <td className="px-1 py-1">
                    <input 
                      ...
                      onChange={(e) => handleItemChange(item.id, 'discountPercentage', parseFloat(e.target.value) || 0)}
                      className="w-full bg-transparent text-sm text-right text-white px-1 py-1 focus:outline-none focus:border-b focus:border-blue-500 appearance-none" 
                      placeholder="0"
                    />
                  </td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6 flex flex-col">
            ...
            {/* Estado de Factura */}
            <div>
              <h2 className="bg-slate-700 text-white font-bold py-1 px-3 rounded-t-lg text-sm uppercase mt-4">ESTADO</h2>
              <div className="border border-slate-700 rounded-b-lg p-4 bg-slate-800/30">
                ...
                >
                  <option value="estimate">Presupuesto Generado</option>
